...
def run(num_windows: int = NUM_WINDOWS, save_weights: str | None = None,
    ...
    logger.info(
        f"[CFG] device={device} | T={TIME_BINS} | DECAY={DECAY} | WARMUP={WARMUP_WINDOWS} "
    ...
    for win_id, seq in enumerate(stream_iter(), 1):
        ...
        if win_id > WARMUP_WINDOWS and comp_debug_remaining > 0 and comp_scores.sum().item() > 0:
            ...
            logger.info(
                f"[DBG][comp] window={win_id} mean={mean_score:.2f} max={best:.2f} "
                f"runner_up={runner_up:.2f} margin={margin:.2f}"
            )
        ...
        with torch.no_grad():
            ...
            target = Ww[0].numel() * 0.05
            scale = (target / mass).clamp(0.5, 2.0)
            Ww.mul_(scale.view(-1, 1, 1, 1))
        ...
        logged_wta = False
        if win_id % 500 == 0 or win_id == total_windows:
